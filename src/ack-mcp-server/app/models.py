...
class ObservabilityContext(BaseModel):
    ...
    sls_project: str = Field(
        description=("The SLS (Log Service) project name for given ACK Cluster")
    )
    sls_log_store: str = Field(
        description=("The SLS (Log Service) log store name for given ACK Cluster")
    )
...
class ClusterInfo(BaseModel):
    ...
    cluster_id: str = Field(description="The unique identifier for the cluster")
    name: str = Field(description="The name of the cluster")
    region_id: str = Field(description="The region where the cluster is deployed")
    state: str = Field(
    ...
    tags: list[dict] = Field(description="List of tags associated with the cluster")
    maintenance_window: dict = Field(description="Maintenance window configuration")

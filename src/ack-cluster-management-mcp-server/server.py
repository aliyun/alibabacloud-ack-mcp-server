...
This server provides ACK cluster management capabilities including:
- Query cluster task status and information
- Create cluster diagnosis tasks  
- Get cluster diagnosis results
"""
...
1. **describe_task_info**: Query ACK cluster task status and information
   - Get detailed information about cluster operations
   - Track task progress and status

2. **create_cluster_diagnosis**: Create cluster diagnosis task for ACK cluster  
   - Initiate comprehensive cluster health checks
   - Identify potential issues and problems

3. **get_cluster_diagnosis_result**: Get cluster diagnosis task result
   - Retrieve diagnosis results and recommendations
   - Get detailed analysis of cluster health
...
def create_mcp_server(config: Optional[Dict[str, Any]] = None) -> FastMCP:
    """Create ACK Cluster Management MCP server instance.
    ...
    ACKClusterManagementHandler(server, allow_write, config)
    
    logger.info(f"ACK Cluster Management MCP Server created successfully on {host}:{port}")
    return server


def main():
    """Run ACK Cluster Management MCP server as standalone application."""
    # 首先加载.env文件
    if DOTENV_AVAILABLE:
        load_dotenv()
    ...
    parser = argparse.ArgumentParser(
        description="ACK Cluster Management MCP Server"
    )
    parser.add_argument(
        "--allow-write",
        action="store_true",
    ...
    logger.info(f"Starting ACK Cluster Management MCP Server (region: {config['region_id']})")

...
class AlibabaSLSProvider(Provider):
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            logger.info(
                f"SLS client initialized successfully for project: {self.project}"
            )
    ...
    def _query_logs(
        self, query: str, start_time: int, end_time: int, params: Dict[str, Any]
    ) -> Dict[str, Any]:
                    ...
                    for log_entry in response.body["data"]:
                        ...
                        if "objectRef" in log_entry:
                            try:
                                ...
                                log_data["objectRef"] = {
                                    "resource": log_entry["objectRef"]
                                }

                        if "responseStatus" in log_entry:
                            try:
                                response_status_data = json.loads(
                                    log_entry["responseStatus"]
                                )
                        ...
                        if "requestObject" in log_entry:
                            try:
                                request_object_data = json.loads(
                                    log_entry["requestObject"]
                                )
                        ...
                        if "responseObject" in log_entry:
                            try:
                                response_object_data = json.loads(
                                    log_entry["responseObject"]
                                )
                        ...
                        log_data["timestamp"] = log_entry.get(
    ...
    def _build_query(self, params: Dict[str, Any]) -> str:
        ...
        if params.get("resource_types") and len(params["resource_types"]) > 0:
            resource_types = [
                f'objectRef.resource: "{rt}"' for rt in params["resource_types"]
            ]

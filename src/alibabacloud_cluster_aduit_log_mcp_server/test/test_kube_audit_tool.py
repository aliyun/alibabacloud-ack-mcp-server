...
class TestKubeAuditTool:
    ...
    async def test_query_audit_log_success(self, mock_context, mock_query_params):
        ...
        # Mock provider
        mock_provider = Mock()
        mock_provider.query_audit_log = AsyncMock(
            return_value={
                "entries": [
                    {
        ...
        with patch.object(
            tool, "_get_provider_from_context", return_value=mock_provider
        ):
    ...
    @pytest.mark.asyncio
    async def test_query_audit_log_provider_error(
        self, mock_context, mock_query_params
    ):

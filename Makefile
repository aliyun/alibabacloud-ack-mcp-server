...
run-http: ## Run the MCP server with Streaming HTTP transport
	PYTHONPATH=src python -m src.main_server --transport http --allow-write --host 127.0.0.1 --port 8000

run-sse: ## Run the MCP server with SSE transport
	PYTHONPATH=src python -m src.main_server --transport sse --allow-write --host 127.0.0.1 --port 8000
...
build-spec: ## Build binary using PyInstaller spec file
	...
	@if [ -f ".venv/bin/activate" ]; then \
		echo "Using virtual environment..."; \
		. .venv/bin/activate && python -m pip install pyinstaller; \
		. .venv/bin/activate && python -m PyInstaller ack-mcp-server.spec; \
	else \
		echo "No virtual environment found, using system Python..."; \
		python3 -m pip install pyinstaller; \
		python3 -m PyInstaller ack-mcp-server.spec; \
	fi
